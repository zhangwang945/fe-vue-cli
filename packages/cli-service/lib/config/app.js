...
module.exports = (api, options) => {
  api.chainWebpack(webpackConfig => {
    ...
    if (!multiPageConfig) {
      ...
      let customizeHtmlPath = api.resolve('src/index.html')
      // 自定义html模板路径
      const projectName = options.projectName
      if (projectName) {
        customizeHtmlPath = api.resolve(`src/pages/${projectName}/index.html`)
      }
      if (fs.existsSync(customizeHtmlPath)) {
        htmlOptions.template = customizeHtmlPath
      } else {
      ...
      const dllConfig = api.service.dllConfig
      webpackConfig
        .plugin('html')
          .use(HTMLPlugin, [htmlOptions])

      dllConfig && webpackConfig
          .plugin('DllReferencePlugin')
            .use(webpack.DllReferencePlugin, [{ manifest: dllConfig.manifest }])

      dllConfig && webpackConfig
            .plugin('AddAssetHtmlPlugin')
              .use(AddAssetHtmlPlugin, [{
                filepath: dllConfig.scriptPath,
                outputPath: 'js',
                publicPath: `${options.publicPath}js`
              }])
      // console.log(api.service.dllConfig)
      // console.log(webpack.DllReferencePlugin)
      // throw 44
      if (!isLegacyBundle) {
