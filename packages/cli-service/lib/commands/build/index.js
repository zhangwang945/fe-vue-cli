...
module.exports = (api, options) => {
  api.registerCommand('build', {
    ...
    const argsEntry = args.entry || args._[0]
    args.entry = customizeResolveEntry(api.service.context, argsEntry) // 自定义解析入口
...
async function build (args, api, options) {
  ...
  if (projectName) {
    options.outputDir += `/${projectName}`
    options.projectName = projectName
    // options.publicPath += `${projectName}/`
  }

  // 处理publicPath
  // override publicPath
  if (process.env.PUBLIC_URL) {
    options.publicPath = process.env.PUBLIC_URL.replace(/([^/])$/, '$1/')
  }
  if (projectName)options.publicPath += `${projectName}/`
